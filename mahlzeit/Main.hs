...
searchPredicate SearchOptions{..} Recipe{..} = 
  let titleOk = maybe True (title `contains`) searchTitle
      ingredientsOk = maybe True (all (\search -> any ((`contains` search) . ingredient) ingredients)) searchIngredients
      tagsOk = maybe True (\ts -> map Data.Text.toLower ts `isSubsequenceOf` map Data.Text.toLower tags) searchTags
   in titleOk && ingredientsOk && tagsOk
...
main = do
  ...
  case command of
    Search options -> do 
      ...
      forM_ recipes $ \(recipeId, recipe) ->
        Data.Text.IO.putStrLn $ shortInfo recipeId recipe
    Display recipeId factor -> do
      recipe <- findById recipeId
      let recipe' = maybe recipe (`rescale` recipe) factor
      Data.Text.IO.putStrLn $ recipeInfo recipe'
